...
bool Button::mouse_button_event (const Vector2i& p, int button, bool down, int modifiers) {
  ...
  // Temp increase button refcount, in case button causes parent window destruction
...
void Button::draw (NVGcontext* ctx) {
  ...
  if (m_icon) {
    ...
    if (nvg_is_font_icon (m_icon))
      ...
      NVGpaint img_paint = nvgImagePattern (ctx, icon_pos.x(), icon_pos.y() - ih/2, iw, ih, 0, m_icon, m_enabled ? 0.5f : 0.25f);
