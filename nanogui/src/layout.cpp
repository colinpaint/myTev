...
BoxLayout::BoxLayout (Orientation orientation, Alignment alignment, int margin, int spacing)
    : m_orientation(orientation), m_alignment(alignment), m_margin(margin), m_spacing(spacing) {}
//}}}
...
Vector2i BoxLayout::preferred_size (NVGcontext *ctx, const Widget *widget) const {
  ...
  for (auto w : widget->children()) {
    ...
    Vector2i target_size (fs[0] ? fs[0] : ps[0],
                          fs[1] ? fs[1] : ps[1]);
...
void AdvancedGridLayout::compute_layout (NVGcontext *ctx, const Widget *widget, std::vector<int> *_grid) const {
  ...
  Vector2i fs_w = widget->fixed_size();
  Vector2i container_size (fs_w[0] ? fs_w[0] : widget->width(), fs_w[1] ? fs_w[1] : widget->height());
    ...
    for (int phase = 0; phase < 2; ++phase) {
      for (auto pair : m_anchor) {
        ...
        if (anchor.pos[axis] + anchor.size[axis] > (int) grid.size())
          throw std::runtime_error ("Advanced grid layout: widget is out of bounds: " + (std::string) anchor);
        ...
        if (target_size <= current_size)
          continue;
        if (total_stretch == 0)
          throw std::runtime_error ("Advanced grid layout: no space to place widget: " + (std::string) anchor);
