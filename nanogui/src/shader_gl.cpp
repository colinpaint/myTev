...
Shader::Shader (RenderPass* render_pass, const std::string &name,
                const std::string &vertex_shader, const std::string &fragment_shader, BlendMode blend_mode)
  ...
  auto register_buffer = [&](BufferType type, const std::string &name, int index, GLenum gl_type) {
     ...
     switch (gl_type) {
         ...
         //{{{
         default:
           throw std::runtime_error("Shader::Shader(): unsupported uniform/attribute type!");
         //}}}
         };
