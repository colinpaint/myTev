...
Task<vector<ImageData>> ExrImageLoader::load(istream& iStream, const fs::path& path, const string& channelSelector, int priority) const {
  ...
  for (size_t partIdx = 0; partIdx < parts.size(); ++partIdx) {
    ...
    if (!data.dataWindow.isValid()) {
      throw invalid_argument{tfm::format ("EXR image has invalid data window: [%d,%d] - [%d,%d]",
                                          data.dataWindow.min.x(), data.dataWindow.min.y(), 
                                          data.dataWindow.max.x(), data.dataWindow.max.y())};
      }

    if (!data.displayWindow.isValid()) {
      throw invalid_argument{tfm::format ("EXR image has invalid display window: [%d,%d] - [%d,%d]",
                                          data.displayWindow.min.x(), data.displayWindow.min.y(), 
                                          data.displayWindow.max.x(), data.displayWindow.max.y())};
      }
