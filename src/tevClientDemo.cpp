...
class IPCChannel {
public:
  enum DisplayDirective : uint8_t { OpenImage = 0,
                                    ReloadImage = 1,
                                    CloseImage = 2,
                                    UpdateImage = 3,
                                    CreateImage = 4,
...
IPCChannel::IPCChannel (const std::string &hostname) {
  ...
  if (split == std::string::npos)
    ErrorExit ("Expected \"host:port\" for display server address. Given \"%s\".", hostname);
...
class DisplayItem {
public:
  DisplayItem (const std::string &title, Point2i resolution,
...
private:
  ...
  struct ImageChannelBuffer {
    ImageChannelBuffer (const std::string &channelName, int nTiles, const std::string &title);
    void SetTileBounds (int x, int y, int width, int height);
    bool SendIfChanged (IPCChannel &channel, int tileIndex);

    std::vector<uint8_t> buffer;
    int tileBoundsOffset = 0;
    int channelValuesOffset = 0;
...
bool DisplayItem::Display (IPCChannel &ipcChannel) {
  ...
  for (int c = 0; c < channelBuffers.size(); ++c) {
    float *ptr = (float *)(channelBuffers[c].buffer.data() + channelBuffers[c].channelValuesOffset);
...
static DisplayItem GetImageDisplayItem (const std::string &title, const Image &image,
  ...
  auto getValues = [=](Bounds2i b, pstd::span<pstd::span<float>> displayValues) {
    int offset = 0;
    for (Point2i p : b) {
      ImageChannelValues v = channelDesc ? image.GetChannels(p, *channelDesc) : image.GetChannels(p);
...
//}}}
//{{{
void DisplayStatic (std::string title, Point2i resolution, std::vector<std::string> channelNames,
                    std::function<void(Bounds2i b, pstd::span<pstd::span<float>>)> getTileValues) {
...
//}}}
//{{{
void DisplayDynamic (std::string title, Point2i resolution, std::vector<std::string> channelNames,
                     std::function<void(Bounds2i b, pstd::span<pstd::span<float>>)> getTileValues) {
