...
namespace tlog {
  ...
  //}}}
  //{{{
  template <typename T> std::string durationToString(T dur) {
  ...
  inline std::string progressBar (uint64_t current, uint64_t total, duration_t duration, int width) {
    ...
    // Time display. Looks like so:   3s/17m03s
